...
.PHONY: unit-test
unit-test: manifests generate fmt vet ## Run unit tests.
...
integration-test-envtest: manifests generate fmt vet envtest ## Run integration tests using envtest.
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" ginkgo -v -github-output -timeout=$(TEST_TIMEOUT) -output-dir=./coverage/integration-tests/ -coverprofile=coverage-envtest.txt -covermode=atomic -coverpkg=all ./internal/controller/test/envtest/

.PHONY: integration-test-k3s
integration-test-k3s: manifests generate fmt vet ## Run integration tests that require a k3s cluster.
	ginkgo -p -v -github-output -timeout=$(TEST_TIMEOUT) \
		-output-dir=./coverage/integration-tests/ -coverprofile=coverage-k3s.txt -covermode=atomic -coverpkg=all \
		./internal/controller/test/k3s/
